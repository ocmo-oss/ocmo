...
describe("resolveActionLabel", () => {
  it("builds action labels", () => {
    ...
    expect(resolveActionLabel(false, "3", { markStable: true })).toBe(
      "Resolve v3 & mark stable",
    );
  });
});
...
describe("emptyResolveArtifactsMessage", () => {
  it("describes empty folder resolve", () => {
    expect(emptyResolveArtifactsMessage({ mode: "folder" })).toBe(
      "No resolvable configs found in this folder.",
    );
  });
  ...
  it("describes empty config resolve", () => {
    expect(emptyResolveArtifactsMessage({ mode: "config" })).toBe(
      "Resolve completed with no output artifacts.",
    );
  });
});
