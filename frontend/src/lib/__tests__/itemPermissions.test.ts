...
describe('itemPermissions', () => {
  it('includes audit operation for configs', () => {
    ...
    expect(allPermissionOps('config', FALLBACK_PERMISSION_ACTIONS)).toContain('config:audit')
  })

  it('maps folder audit to folder:audit', () => {
    expect(permissionOpsForType('folder', FALLBACK_PERMISSION_ACTIONS).audit).toBe('folder:audit')
  })

  it('maps resolver audit to resolver:audit', () => {
    expect(permissionOpsForType('resolver', FALLBACK_PERMISSION_ACTIONS).audit).toBe('resolver:audit')
  })
})
