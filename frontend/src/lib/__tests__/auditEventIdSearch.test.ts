...
import { applyAuditEventIdFilter, extractAuditEventIdInput, isCompleteAuditEventId } from '../auditEventIdSearch'
...
describe('extractAuditEventIdInput', () => {
  ...
  it('extracts id from copied notification body', () => {
    expect(extractAuditEventIdInput(
...
describe('applyAuditEventIdFilter', () => {
  it('drops incomplete event_id from api filters', () => {
    expect(applyAuditEventIdFilter({ event_id: '550e8400-e29b', auth_email: 'a@b.c' })).toEqual({
      auth_email: 'a@b.c',
    })
  })

  it('keeps complete event_id', () => {
    expect(applyAuditEventIdFilter({ event_id: EVENT_ID })).toEqual({ event_id: EVENT_ID })
  })
})
