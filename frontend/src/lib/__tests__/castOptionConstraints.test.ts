...
import { applyCastOptionChange, getCastOptionFieldState } from '../castOptionConstraints'
...
const envProperties: Record<string, JsonSchemaProperty> = {
  ...
  list_format: { default: 'indexed', enum: ['indexed', 'joined', 'json', 'space'] },
  list_separator: { default: ',', 'x-ocmo-enabled-when': { list_format: 'joined' } },
}
...
describe('castOptionConstraints', () => {
  ...
  it('clears incompatible fields when enabling uppercase', () => {
    const next = applyCastOptionChange(
      'uppercase',
      true,
      envProperties,
      { lowercase: true },
