...
import { buildConfigEditorSchema, filterMetadataSchemaForEditor } from '../configEditorSchema'
...
const mockOcmoSchema = {
  ...
  properties: {
    ...
    is_json_schema: { type: 'boolean', description: 'JSON Schema document marker' },
  },
}
...
describe('buildConfigEditorSchema json schema mode', () => {
  ...
  it('uses consumer data schema in normal mode', () => {
    ...
    const composed = buildConfigEditorSchema('_ocmo', mockOcmoSchema, dataSchema, {
  ...
  it('uses explicit data schema for builtin policy documents', () => {
    ...
    const composed = buildConfigEditorSchema('_ocmo', mockOcmoSchema, policySchema, {
      isJsonSchemaMode: true,
