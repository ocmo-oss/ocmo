...
describe('datetime', () => {
  ...
  it('omits year for the current calendar year', () => {
    expect(formatUserDateTime('2026-03-15T10:00:00.000Z')).toBe('Mar 15, 10:00')
    expect(formatUserDateTimeShort('2026-03-15T10:00:00.000Z')).toBe('Mar 15 10:00')
    expect(userDateTimeFormatPattern(new Date('2026-03-15T10:00:00.000Z'), 'date')).toBe('MMM d')
  })

  it('includes year when not the current calendar year', () => {
    expect(formatUserDateTime('2025-03-15T10:00:00.000Z')).toBe('Mar 15, 2025, 10:00')
    expect(formatUserDateTimeShort('2025-03-15T10:00:00.000Z')).toBe('Mar 15, 2025 10:00')
    expect(userDateTimeFormatPattern(new Date('2025-03-15T10:00:00.000Z'), 'date')).toBe('MMM d, yyyy')
  })

  it('formats chart axis labels without AM/PM', () => {
    const hour = 60 * 60 * 1000
    const day = 24 * hour
    expect(formatUserDateTimeAxis('2026-08-04T12:00:00.000Z', hour)).toMatch(/Aug 4 \d{2}:\d{2}/)
    expect(formatUserDateTimeAxis('2025-08-04T12:00:00.000Z', day)).toBe('Aug 4, 2025')
    expect(formatUserDateTimeAxis('2026-08-04T12:00:00.000Z', hour / 2)).toMatch(/^\d{2}:\d{2}$/)
  })

  it('formats chart bucket tooltips in 24h format', () => {
    const hour = 60 * 60 * 1000
    expect(formatUserDateTimeBucketRange('2026-08-04T10:00:00.000Z', hour)).toBe('Aug 4, 10:00-11:00')
    expect(formatUserDateTimeBucketRange('2025-08-04T10:00:00.000Z', hour)).toBe('Aug 4, 2025, 10:00-11:00')
  })
  ...
  it('converts datetime-local wall time to UTC ISO', () => {
    ...
    expect(localDateTimeInputToUtcIso('2026-08-04T15:30')).toBe('2026-08-04T12:30:00.000Z')
    vi.unstubAllGlobals()
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-08-04T12:00:00.000Z'))
  })
  ...
  it('converts UTC ISO to datetime-local input value', () => {
    ...
    expect(utcIsoToLocalDateTimeInput('2026-08-04T12:30:00.000Z')).toBe('2026-08-04T15:30')
    expect(localDateTimeInputFromDate(new MockDate('2026-08-04T12:30:00.000Z'))).toBe('2026-08-04T15:30')
    vi.unstubAllGlobals()
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-08-04T12:00:00.000Z'))
  })
  ...
  it('computes minimum future datetime-local input from an offset', () => {
    ...
    const expected = localDateTimeInputFromDate(new Date(now.getTime() + 30 * 60 * 1000))
  ...
  it('validates optional future datetime-local input', () => {
    ...
    expect(isOptionalFutureLocalDateTimeInput(
