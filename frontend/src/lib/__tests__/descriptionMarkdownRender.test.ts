...
describe('isExternalMarkdownLink', () => {
  ...
  it('treats off-site http links as external', () => {
    expect(isExternalMarkdownLink('https://example.com/docs', pageOrigin)).toBe(true)
  })

  it('treats same-origin http links as internal', () => {
    expect(isExternalMarkdownLink(`${pageOrigin}/docs`, pageOrigin)).toBe(false)
  })
  ...
  it('does not treat mailto links as external http links', () => {
    expect(isExternalMarkdownLink('mailto:hello@example.com', pageOrigin)).toBe(false)
  })
})
