...
describe("buildDestinationPathFromFolder", () => {
  it("appends the item name under the selected folder", () => {
    ...
    expect(buildDestinationPathFromFolder("moved/sub", "cfg1")).toBe(
      "moved/sub/cfg1",
    );
  });
});
...
describe("validateRelocationTargetPath", () => {
  it("rejects moving an item under its own path", () => {
    expect(
      validateRelocationTargetPath("configX", "configX/configY", "move"),
    ).toBe("Destination cannot be under the source path");
  });

  it("allows copying to a child path", () => {
    expect(
      validateRelocationTargetPath("configX", "configX/configY", "copy"),
    ).toBeUndefined();
  });
});
