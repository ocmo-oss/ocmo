...
describe('lineKey', () => {
  it('parses JSON Schema keys that start with $', () => {
    expect(lineKey('$schema: https://json-schema.org/draft/2020-12/schema')).toBe('$schema')
...
describe('escapeMonacoSnippetDollars', () => {
  it('escapes $schema but preserves tab stops', () => {
    expect(escapeMonacoSnippetDollars(JSON_SCHEMA_ROOT_SNIPPET_BODY)).toContain('\\$schema:')
    expect(escapeMonacoSnippetDollars(JSON_SCHEMA_ROOT_SNIPPET_BODY)).toContain('${1:Data title}')
  })
})

describe('shouldOfferJsonSchemaRootSnippet', () => {
  it('returns false when $schema is present', () => {
    const model = textModel('$schema: https://json-schema.org/draft/2020-12/schema\n')
