...
import { formatAuditOperation, inferOperationFromLegacyFields } from '../auditEvent'
...
describe('formatAuditOperation', () => {
  it('prefers persisted operation field', () => {
    expect(formatAuditOperation(event({ operation: 'Set tag' }))).toBe('Set tag')
  })

  it('falls back for resolve_request without operation', () => {
    expect(
      formatAuditOperation(
        event({ event_kind: 'resolve_request', object_id: 'app/cfg', operation: null }),
      ),
    ).toBe('Resolve')
  })
