...
export function isJsonSchemaConfigContent(content: string, metadataKey = DEFAULT_CONFIG_METADATA_KEY): boolean {
...
export function hasOcmoRenderConfiguration(content: string, metadataKey = DEFAULT_CONFIG_METADATA_KEY): boolean {
  const block = extractOcmoMetadataBlock(content, metadataKey)
  if (!block) return false
  return /^\s*render:\s*(\n|\S)/m.test(block)
}

function extractOcmoMetadataBlock(content: string, metadataKey: string): string | undefined {
...
function extractNestedYamlBlock(parentBlock: string, key: string): string | undefined {
...
export function hasManualPropagation(content: string, metadataKey = DEFAULT_CONFIG_METADATA_KEY): boolean {
  if (!content.trim()) return false

  try {
    const root = readYamlMapping(parseDocument(content, { strict: false }).toJS())
