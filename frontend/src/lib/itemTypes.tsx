...
export function ItemIcon({
  ...
  const icon = type === 'secret' ? (
    ...
    (() => {
      ...
      return (
        <Icon
          className={cn(sizeClass[size], ITEM_TYPE_ICON_CLASS[type], className)}
          aria-hidden="true"
        />
      )
    })()
  ...
  const labeled = (
    <span
      className={cn(sizeClass[size], 'inline-flex shrink-0 items-center justify-center')}
  ...
  return (
    <Tooltip content={ITEM_TYPE_LABELS[type]}>
      {labeled}
    </Tooltip>
  )
}
