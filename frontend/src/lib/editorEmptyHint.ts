...
function isSuggestWidgetVisible(editorInstance: editor.IStandaloneCodeEditor): boolean {
...
export function installEditorEmptyHint(
  ...
  const shouldShow = () => {
    if (editorInstance.getOption(monaco.editor.EditorOption.readOnly)) return false
