...
const mockOcmoMetadataSchema = {
  ...
  properties: {
    ...
    is_json_schema: { type: 'boolean', description: 'Mark body as JSON Schema' },
  },
}
...
function summarise(items: Array<{ label: unknown; sortText?: unknown; insertText: unknown; kind: unknown }>): SuggestionSummary[] {
...
async function getSuggestions(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const result = __testing.buildYamlCompletionSuggestions(monacoStub as any, schema, model as any, position as any, null, paramOptions ?? null)
...
describe('property keys inside actors[] kind: User', () => {
  ...
  it('suggests "claims" key under kind: User', async () => {
    const items = await getSuggestions(permissionsSchema, yaml, claimsLine, claimsCol)
    expect(items).toMatchSnapshot()
    const labels = items.map(i => i.label)
    expect(labels.some(l => l === 'claims' || l.startsWith('claims'))).toBe(true)
  })
})
...
describe('actions enum array items', () => {
  ...
  it('suggests action enum values on empty array item line', async () => {
    const items = await getSuggestions(permissionsSchema, yaml, actionsLine, actionsCol)
    expect(items).toMatchSnapshot()
  })
  ...
  it('adds leading space to insertText when line has bare dash (no trailing space)', async () => {
    ...
    const bareItems = await getSuggestions(permissionsSchema, bareYaml, 8, bareCol)
...
describe('snippet variant sort order', () => {
  it('oneOf snippet variants use z-prefixed sortText', async () => {
    ...
    const snippets = items.filter(i => i.kind === monacoStub.languages.CompletionItemKind.Snippet)
  ...
  it('enum/discriminator suggestions sort before snippets', async () => {
    ...
    const items = await getSuggestions(permissionsSchema, yaml, 2, yaml.split('\n')[1].length + 1)
    const enums = items.filter(i => i.kind === monacoStub.languages.CompletionItemKind.EnumMember)
    const snippets = items.filter(i => i.kind === monacoStub.languages.CompletionItemKind.Snippet)
...
describe('formatCompletionPreviewDetail', () => {
  ...
  it('keeps unrelated preview text intact', () => {
    expect(formatCompletionPreviewDetail('kind: User', '- kind: User\n  claims:')).toBe('- kind: User …')
  })
})
...
describe('json schema config editor mode', () => {
  ...
  it('suggests only untyped keywords under a property before type is set', async () => {
    ...
    const labels = items.map(i => i.label)
    expect(labels).toEqual(expect.arrayContaining(['type', 'title', 'description']))
