...
import { isScalarArrayItemLine, arrayItemValueStartColumn, stripYamlScalarQuotes as stripYamlQuotes } from './lineSyntax'

type JsonSchema = Record<string, unknown>

export type OcmoUriReferenceScope = 'config' | 'resolver' | 'resource'

const CONFIG_REFERENCE_ITEM_TYPES = new Set<ItemType>(['config', 'template', 'secret'])
...
const searchCache = new Map<string, { expires: number; items: TreeNavigationNode[] }>()
const CACHE_TTL_MS = 10_000


function unwrapSchemaForUriRef(schema: JsonSchema | null, root: JsonSchema): JsonSchema | null {
...
export function hasUriReferenceFormat(schema: JsonSchema | null, root: JsonSchema): boolean {
  const resolved = unwrapSchemaForUriRef(schema, root)
  if (!resolved) return false
  if (resolved.format === 'uri-reference') return true

  if (Array.isArray(resolved.anyOf)) {
    return resolved.anyOf.some(item => asObject(item)?.format === 'uri-reference')
  }

  if (Array.isArray(resolved.oneOf)) {
    return resolved.oneOf.some(item => asObject(item)?.format === 'uri-reference')
  }
...
export function isInOcmoMetadata(objectPath: string[], metadataKey: string): boolean {
...
export function extractTypedUriReference(
  ...
  if (ctx.kind === 'property-value' && ctx.valuePropertyKey) {
    ...
    while (valueStartColumn <= line.length && /\s/.test(line[valueStartColumn - 1] ?? '')) {
      valueStartColumn += 1
    }
  ...
  const raw = stripYamlQuotes(valueTextBeforeCursor(model, position, valueStartColumn))
...
function resolvedPrefixParts(resolvedPrefix: string): { parent: string; partial: string } {
...
async function listItemsMatchingPrefix(
  ...
  return filterReferenceItems(candidates, resolvedPrefix, currentConfigPath, scope)
}
...
async function searchReferenceItems(
  ...
  if (!absoluteOnly && isDirectoryBrowsePrefix(typedPathPart, resolvedPrefix)) {
    try {
      const descendants = await listFolderDescendants(namespace, resolvedPrefix, signal)
      return filterReferenceItems(descendants, resolvedPrefix, currentConfigPath, scope, {
  ...
  if (cached && cached.expires > Date.now()) {
    return filterReferenceItems(cached.items, resolvedPrefix, currentConfigPath, scope)
  }
  ...
  searchCache.set(cacheKey, { expires: Date.now() + CACHE_TTL_MS, items: referenceItems })
  return filterReferenceItems(referenceItems, resolvedPrefix, currentConfigPath, scope)
}

function formatArrayItemUriInsertText(line: string, insertText: string): string {
...
function requiresAbsoluteReferences(
  ...
  return options.configPath === '_permissions'
...
function shouldSuppressUriReferenceSuggestions(
  ...
  if (suggestions.length === 1 && suggestions[0].path === resolvedPrefix) return true
  return false
}
...
export function shouldSuggestUriReferences(
  ...
  const allowEmptyBrowse = Boolean(options.allowOutsideMetadata)
  ...
  const resolvedPrefix = resolveRelativeOcmoPath(options.configPath, typed.pathPart)
  return isUriReferenceSearchReady(resolvedPrefix)
...
export async function buildSecretPathSuggestions(
  ...
  const typed = extractTypedUriReference(model, position, ctx)
    ?? { raw: '', pathPart: '', suffix: '', resolvedPrefix: '' }
  if (!shouldSuggestSecretPathReferences(options as UriReferenceCompletionOptions, typed)) {
    return []
  }
  ...
  const suggestions = expandPathSuggestions(items, 'config')
    .filter(suggestion => suggestion.type === 'secret')
  if (suggestions.length === 0) return []
  if (shouldSuppressUriReferenceSuggestions(suggestions, resolved.resolvedPrefix)) {
    return []
  }

  const uriOptions = options as UriReferenceCompletionOptions
  return suggestions.map((suggestion) => {
    const insertText = formatYamlScalar(formatReferenceInsertText(
      uriOptions,
      'config',
      suggestion.path,
      resolved.pathPart,
...
export async function buildUriReferenceSuggestions(
  ...
  return suggestions.map((suggestion) => {
    let insertText = formatYamlScalar(formatReferenceInsertText(
      options,
      scope,
      suggestion.path,
      resolved.pathPart,
