...
function isParameterNameKey(key: string): boolean {
  return (
    PARAMETER_NAME_PATTERN.test(key) && !RESERVED_PARAMETER_FIELD_NAMES.has(key)
  );
}
...
export function isAtParameterFieldRow(
  ...
  const rowIndent = explicitPropertyRowIndent(
    model,
    position.lineNumber,
    position.column,
  );
...
export function parameterDeclarationMissingFields(
  ...
  const schema = rootSchema
    ? resolveParameterDeclarationSchema(rootSchema, objectPath)
    : null;
...
function propertyValueEndColumn(
  line: string,
...
function fullPropertyValueAtPosition(
  ...
  const typedBefore = stripYamlScalarQuotes(
...
export function detectOcmoParameterValueContext(
  ...
  return detectOcmoParameterDeclarationContext(
    model,
    position,
    ctx,
    metadataKey,
...
export function readOcmoParameterType(
  ...
  return parseOcmoParameters(model.getValue(), metadataKey).find(
...
export function shouldSuggestParameterDeclarationFields(
  ...
  return (
    parameterDeclarationMissingFields(
      rootSchema,
      declaration.objectPath,
      existingKeys,
    ).length > 0
...
export function shouldSuggestParameterValue(
  ...
  const declaration = detectOcmoParameterValueContext(
    model,
    position,
    ctx,
    metadataKey,
  ...
  if (parameterType === "projected") {
    const { typedBefore, fullValue } = fullPropertyValueAtPosition(
      model,
      position,
    );
...
export function buildParameterProjectedValueSuggestions(
  ...
  const completionRange =
    range ?? propertyValueCompletionRange(model, position);
  ...
  return PROJECTED_PARAMETER_SELECTORS.filter((selector) =>
    shouldShowProjectedSelectorSuggestion(
      selector.label,
      typedBefore,
      fullValue,
