...
const unwrapForBuildCache = new WeakMap<
  JsonSchema,
  WeakMap<JsonSchema, JsonSchema | null>
>();

/** Resolves schema for nested snippet generation (picks first oneOf branch). Memoized per root. */
function unwrapSchemaForBuild(
  ...
  const result = resolveSchemaForBuild(
    schema,
    root,
    "",
...
function snippetVariantLabel(
  ...
  const parts: string[] = [];
  const sortedChoices = [...oneOfChoices.entries()].sort(([a], [b]) =>
    a.localeCompare(b),
  );
...
function isObjectSchema(schema: JsonSchema): boolean {
  return (
    schema.type === "object" ||
    Boolean(asObject(schema.properties)) ||
...
function schemaFirstArrayItemExample(arraySchema: JsonSchema): string | null {
  if (!Array.isArray(arraySchema.examples) || arraySchema.examples.length === 0)
    return null;
...
function hasScalarValueSuggestions(
  schema: JsonSchema,
  root: JsonSchema,
): boolean {
  return (
    hasEnumSchema(schema, root) ||
    hasBooleanSchema(schema, root) ||
    hasScalarAnyOfSchema(schema, root)
  );
}

function scalarSuggestionValues(
  schema: JsonSchema,
  root: JsonSchema,
): string[] {
...
function keysForSnippet(
  ...
  if (options.depth === "full") {
    ...
    const minProps =
      typeof objectSchema.minProperties === "number"
...
function buildScalarFieldText(
  ...
  if (prop.const !== undefined) {
    return {
      text: `${key}: ${formatYamlScalarValue(prop.const, kind)}`,
      nextTab: tab,
    };
  }
...
function buildArrayItemLines(
  ...
  if (isObjectSchema(item)) {
    ...
    if (firstKey === "__additional__") {
      ...
      const placeholder = addSchema
        ? scalarPlaceholder(addSchema, "key")
        : "value";
      const formatted = formatYamlScalarSnippet(
        tab,
        placeholder,
        yamlScalarKindFromSchema(addSchema),
      );
  ...
  const placeholder = scalarPlaceholder(
    item,
    arrayKey ?? "item",
    parentArraySchema,
  );
...
function buildPropertyLines(
  ...
  if (isArraySchema(prop)) {
    ...
    return {
      lines: arrayLines.lines.map((line, index) =>
        index === 0 ? `${pad}${line}` : line,
      ),
...
function buildObjectLines(
  ...
  for (const key of keys) {
    ...
    const fieldLines = buildPropertyLines(
      ...
      childSchemaPath(
        schemaPath,
        key === "__additional__" ? "__additional__" : key,
      ),
    );
    lines.push(...fieldLines.lines);
    tab = fieldLines.nextTab;
...
function snippetPreviewText(insertText: string): string {
  return insertText.replace(/\$\{\d+:([^}]*)\}/g, "$1").replace(/\$\d+/g, "");
}
...
function formatCompletionPreviewDetail(
  primaryLabel: string,
  preview: string,
): string {
...
function resolveOneOfPropertySchema(
  ...
  if (enumValues.length > 0) {
    ...
    const description = propSchemas.find(
...
function resolveTargetSchema(
  ...
  if (ctx.kind === "array-item") {
    const arraySchema = schemaAtPath(
      root,
      ctx.objectPath,
  ...
  if (ctx.kind === "property-value" && ctx.valuePropertyKey) {
    ...
    if (model && lineNumber && isCastOptionsObjectPath(ctx.objectPath)) {
      const castOptions = resolveCastOptionsFromContext(
        root,
        model,
        lineNumber,
        ctx.objectPath,
  ...
  if (inArrayItemObject && model && lineNumber) {
    const parentSchema = schemaAtPath(
      root,
      ctx.objectPath,
      schemaPathOptions(ctx),
    );
...
function inferredPropertyRowIndent(
  model: Monaco.editor.ITextModel,
  lineNumber: number,
): number {
...
function expectedPropertyRowIndent(
  ...
  if (insideArrayItem) {
    ...
    const itemIndent = enclosingArrayItemKeyLineIndent(
      model,
      lineNumber,
      rowIndent,
    );
...
function isAtPropertyRow(
  ...
  const effective = effectivePropertyRowIndent(
    model,
    position.lineNumber,
    position.column,
  );
...
function isAtArrayElementRow(
  ...
  const expected = expectedArrayElementRowIndent(
    model,
    position.lineNumber,
    objectPath,
  );
...
function isWithinArrayItemObject(
  ...
  const arrayItemIndent = enclosingArrayItemKeyLineIndent(
    model,
    lineNumber,
    rowIndent,
  );
...
function existingKeysInScope(
  ...
  const scopedInsideArrayItem =
    insideArrayItem || isWithinArrayItemObject(model, lineNumber, column);
...
function completionContext(
  ...
  const {
    objectPath,
    insideArrayItem: parentInsideArray,
    arrayItemEnterBefore,
  } = parentYamlContext(model, position.lineNumber, position.column);
  const propertyRowSpaces = effectivePropertyRowIndent(
    model,
    position.lineNumber,
    position.column,
  );
  const indentLevel = Math.floor(propertyRowSpaces / 2);
  const insideArrayItem =
    parentInsideArray ||
  ...
  if (/^\s*-\s*/.test(line)) {
    ...
    if (itemKey && localColon >= 0) {
      const valueStartColumn =
        afterDashStart + afterDash.slice(0, localColon + 1).length + 1;
 ...
 */
function formatPropertyInsertForSnippet(
  insertText: string,
  rowSpaces: number,
): string {
...
function schemaMapExampleEntries(
  mapSchema: JsonSchema,
): ScalarMapExampleEntry[] {
...
function castFormatDefault(castSchema: JsonSchema, root: JsonSchema): string {
  ...
  const formatResolved = formatProp
    ? unwrapSchemaForBuild(formatProp, root)
    : null;
...
function resolveCastOptionsSchema(
  root: JsonSchema,
  format: string,
): JsonSchema | null {
...
function mapValueSchema(
  ...
  if (additional && typeof additional === "object") {
    ...
    if (
      addSchema &&
      isObjectSchema(addSchema) &&
      asObject(addSchema.properties)
    ) {
      return addSchema;
    }
  }
...
function buildScalarMapEntrySuggestion(
  ...
  const kind =
    placeholderOverride?.kind ?? yamlScalarKindFromSchema(valueSchema);
  const placeholder =
    placeholderOverride?.value ?? scalarPlaceholder(valueSchema, mapKey);
  ...
  return {
    ...
    documentation: completionDocumentation(
      mapDescription ?? schemaDescription(valueSchema),
    ),
  };
}
...
function propertySuggestionsForObject(
  ...
  const properties = asObject(resolved.properties) ?? {};
  const fixedEntries = Object.entries(properties).filter(
    ([name]) => !existingKeys.has(name),
  );
  ...
  if (fixedEntries.length > 0) {
    return fixedEntries.flatMap(([name, propSchema]) => {
      ...
      const snippetDepth: SnippetDepth =
        propResolved &&
      ...
      return variants.map((variant) => {
        const label =
          branchLabel && variants.length === 1 && !hasOneOf(prop, root)
            ? `${name} (${branchLabel})`
            : variant.label;
        const finalized = finalizePropertyInsertText(
        ...
        return {
          ...
          detail: variant.isSnippet
            ...
            : typeof prop.type === "string"
              ...
              : undefined,
          documentation: completionDocumentation(
            variant.description ?? fieldDescription,
          ),
  ...
  const scalarMapValue = mapScalarValueSchema(resolved, root);
  if (scalarMapValue) {
    const examples = schemaMapExampleEntries(resolved).filter(
      (entry) => !existingKeys.has(entry.key),
    );
    if (examples.length > 0) {
      return examples.map((entry) =>
        buildScalarMapEntrySuggestion(
          monaco,
          entry.key,
          scalarMapValue,
          range,
...
function propertySnippetVariants(
  ...
  const variants = buildSnippetVariants(resolved, root, "", (options) => {
    const result = buildPropertyInsertText(
      name,
      resolved,
      root,
      indentLevel,
...
function oneOfConstDiscriminatorSuggestions(
  ...
  return values.map((value, index) => ({
    label: {
      label: `${discriminatorKey}: ${value}`,
      description: discriminatorKey,
    },
 ...
 */
function formatNestedSnippetInsert(
  lines: string[],
  anchorSpaces: number,
): string {
...
function formatArrayItemInsertText(
  bodyLines: string[],
  lineContent: string,
): string {
  ...
  const head =
    needsLeadingSpace && firstContent ? ` ${firstContent}` : firstContent;
...
function valueSuggestions(
  ...
  if (branches.length > 1) {
    ...
    for (const [index, branch] of branches.entries()) {
      ...
      for (const item of valueSuggestionsForBranch(
        monaco,
        branch,
        root,
        range,
...
function valueSuggestionsForBranch(
  ...
  if (Array.isArray(resolved.anyOf) && !Array.isArray(resolved.enum)) {
    ...
    for (const branch of resolved.anyOf) {
      ...
      if (!item) continue;
      for (const suggestion of valueSuggestionsForBranch(
        monaco,
        item,
        root,
        range,
  ...
  const valueDocumentation = completionDocumentation(
    schemaDescription(resolved),
  );
  const suggestions: Monaco.languages.CompletionItem[] = [];
...
function scalarArrayItemValueSuggestions(
  ...
  return valueSuggestions(monaco, itemSchema, root, range, typed).map(
    (item) => ({
      ...item,
      insertText: formatScalarArrayItemInsertText(
        lineContent,
...
function analyzePosition(
  ...
  const indentPrefix = leadingIndentPrefix(
    model,
    position.lineNumber,
    position.column,
  );
...
function buildYamlCompletionSuggestions(
  ...
  const { ctx, targetSchema, lineContent, range, indentPrefix } =
    analyzePosition(model, position, rootSchema);
  ...
  if (
    ...
    const parameterSchema = resolveParameterDeclarationSchema(
      rootSchema,
    ...
    if (metadataKey ?? declaration?.objectPath[0]) {
      ...
      if (valueDeclaration) {
        ...
        if (parameterType === "projected") {
          return buildParameterProjectedValueSuggestions(
            monaco,
            model,
            position,
          );
  ...
  if (ctx.kind === "array-item") {
    ...
    const scalarUriReference =
      hasUriReferenceFormat(targetSchema, rootSchema) &&
      uriOptions &&
  ...
  const onPropertyKeyLine =
    lineKey(lineContent) !== null && !/^\s*-/.test(lineContent.trim());
  if (
    !onPropertyKeyLine &&
    isAtArrayElementRow(model, position, ctx.objectPath)
  ) {
    ...
    if (arrayParent && isArraySchema(arrayParent)) {
      ...
      if (items) {
        ...
        const arrayRange = completionRange(model, position, {
          ...ctx,
          kind: "array-item",
        });
        return syncArrayElementSuggestions(
...
function hasYamlCompletionSuggestions(
  ...
  if (
    paramOptions &&
    shouldSuggestParameterPlaceholders(model, position, ctx, paramOptions)
  ) {
    return true;
  }
  const metadataKey = paramOptions?.metadataKey ?? ctx.objectPath[0] ?? null;
  ...
  if (
    ctx.kind === "property-key" &&
    targetSchema &&
    isAtPropertyRow(model, position, ctx)
  ) {
    if (
      !canSuggestInParameterDeclarationObject(
        model,
    ...
    const properties = asObject(
      unwrapSchema(targetSchema, rootSchema)?.properties,
    );
    if (
      properties &&
      Object.keys(properties).some((key) => !existingKeys.has(key))
    ) {
      return true;
    }
  }
...
export function shouldAutoTriggerYamlSuggest(
  ...
  if (before.trim().length > 0 && !/:\s*$/.test(before)) {
    ...
    if (
      ...
      const targetSchema = analyzePosition(
        model,
        position,
        rootSchema,
      ).targetSchema;
      if (targetSchema) {
        ...
        if (hasScalarTrigger(targetSchema, rootSchema, ctx, model, position))
          return true;
      }
    }
