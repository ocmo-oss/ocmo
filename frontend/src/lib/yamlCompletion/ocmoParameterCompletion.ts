...
export function parseOcmoParameterNames(
  ...
  return parseOcmoParameters(yamlContent, metadataKey).map(
    (param) => param.name,
  );
}
...
export function detectParameterPlaceholderContext(
  ...
  return {
    ...
    replaceEndColumn: hasClosingBrace ? position.column : position.column,
    valueSpan,
    hasClosingBrace,
  };
}
...
export function buildParameterPlaceholderSuggestions(
  ...
  const placeholderCtx = detectParameterPlaceholderContext(
    model,
    position,
    ctx,
  );
  ...
  return matches.map((param, index) => {
    ...
    const newContent =
      valueSpan.content.slice(0, replaceStartInContent) +
      placeholderBody +
      afterSuffix;
    const insertText = formatParameterValueForInsert(
      newContent,
      valueSpan.quote,
    );
    ...
    if (documentationParts.length === 0) {
      documentationParts.push(
        `Declared in \`${options.metadataKey}.parameters\`.`,
      );
    }
