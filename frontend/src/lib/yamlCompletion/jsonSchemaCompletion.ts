...
export const JSON_SCHEMA_ROOT_SNIPPET_LABEL =
  "JSON Schema document (base structure)";
...
export function hasYamlRootKey(
  model: Monaco.editor.ITextModel,
  key: string,
): boolean {
...
export function hasJsonSchemaBaseStructure(
  model: Monaco.editor.ITextModel,
): boolean {
...
export function shouldOfferJsonSchemaRootSnippet(
  model: Monaco.editor.ITextModel,
): boolean {
...
export function buildJsonSchemaRootSnippetSuggestion(
  ...
  return {
    ...
    insertText: escapeMonacoSnippetDollars(
      indentPrefix + JSON_SCHEMA_ROOT_SNIPPET_BODY,
    ),
    insertTextRules:
    ...
    documentation: {
      value:
        "Insert a draft 2020-12 object schema with empty `required` and `properties`.",
      isTrusted: true,
    },
  };
}
