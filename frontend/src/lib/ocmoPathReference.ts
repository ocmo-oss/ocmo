...
export function splitOcmoReference(ref: string): { pathPart: string; suffix: string } {
...
export function resolveRelativeOcmoPath(baseConfigPath: string, refPath: string): string {
  const isRelative = refPath === '.' || refPath === '..' || refPath.startsWith('./') || refPath.startsWith('../')
  if (isRelative) {
...
export function isRelativePath(refPath: string): boolean {
  return refPath === '.' || refPath === '..' || refPath.startsWith('./') || refPath.startsWith('../')
}
...
export function isDirectoryBrowsePrefix(typedPathPart: string, resolvedPrefix: string): boolean {
  if (!resolvedPrefix) return false
  return isRelativeDirectoryBrowse(typedPathPart) || typedPathPart.endsWith('/')
}
...
export function isUnderFolderPrefix(itemPath: string, folderPath: string): boolean {
...
export function toRelativeOcmoPath(baseConfigPath: string, absolutePath: string): string {
...
export function formatOcmoReferenceInsert(
  ...
  const path = referenceStyle(typedPathPart) === 'relative'
...
export function isUriReferenceSearchReady(resolvedPrefix: string): boolean {
  ...
  return segments.length > 1 && normalized.replace(/\//g, '').length >= MIN_URI_REFERENCE_SEARCH_LEN
}
 ...
 */
export function pathMatchesTypedPrefix(itemPath: string, resolvedPrefix: string): boolean {
  const normalizedPrefix = resolvedPrefix.replace(/\/+$/, '')
  if (!normalizedPrefix) return true

  if (itemPath === normalizedPrefix || itemPath.startsWith(`${normalizedPrefix}/`)) {
    return true
  }
...
export function pathMatchesPrefix(itemPath: string, resolvedPrefix: string): boolean {
