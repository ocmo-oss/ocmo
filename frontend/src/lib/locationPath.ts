...
export function buildDestinationPathFromFolder(folderPath: string, itemName: string): string {
...
export function isPathUnderPrefix(itemPath: string, prefixPath: string): boolean {
...
export function validateRelocationTargetPath(
  ...
  if (mode === 'move' && isPathUnderPrefix(normalizedTarget, normalizedSource)) {
