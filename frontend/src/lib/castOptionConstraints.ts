...
export function getCastOptionFieldState(
  ...
  const incompatibleWith = prop["x-ocmo-incompatible-with"] as
    string[] | undefined;
  if (
    incompatibleWith?.some(
      (other) => effectiveValue(values, other, properties[other]) === true,
    )
  ) {
    const blocker = incompatibleWith.find(
      (other) => effectiveValue(values, other, properties[other]) === true,
    );
    const blockerLabel = blocker
      ? (properties[blocker]?.title ?? blocker.replace(/_/g, " "))
      : "another option";
    return { disabled: true, reason: `Incompatible with ${blockerLabel}` };
  }

  const enabledWhen = prop["x-ocmo-enabled-when"] as
    Record<string, string> | undefined;
  if (!matchesEnabledWhen(values, properties, enabledWhen)) {
    const requirement = enabledWhen
      ? Object.entries(enabledWhen)
          .map(([key, value]) => `${key}=${value}`)
          .join(", ")
      : "";
    return {
      disabled: true,
      reason: requirement ? `Only applies when ${requirement}` : undefined,
    };
  }
