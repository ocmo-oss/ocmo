...
export function inferOperationFromLegacyFields(event: AuditEvent): string {
  ...
  if (event.event_kind === 'resolve_participant') return 'Referenced in resolve'
  ...
  for (const [pattern, label] of ENDPOINT_ACTIONS) {
    ...
    if (label === 'Set tag' && event.http_method === 'DELETE') return 'Delete tag'
    if (label === 'Lock') {
  ...
  if (event.http_method === 'PUT' || event.http_method === 'PATCH') return 'Update item'
...
export function auditCategoryToApiFilter(category: AuditCategory): { category?: string } {
