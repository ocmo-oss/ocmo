...
export function parse(tokens: Token[]): Program {
  ...
  function parseForStatement(): For {
    ...
    if (!(
      loopVariable instanceof Identifier || loopVariable instanceof TupleLiteral
    )) {
    ...
    while (is(TOKEN_TYPES.Dot) || is(TOKEN_TYPES.OpenSquareBracket)) {
      ...
      if (computed) {
        ...
        if (
          ...
          property.type !== "IntegerLiteral"
        ) {
          throw new SyntaxError(
            `Expected identifier or integer following dot operator`,
          );
        }
      }
  ...
  function parsePrimaryExpression(): Statement {
    ...
    switch (token.type) {
      case TOKEN_TYPES.NumericLiteral: {
        ...
        return num.includes(".")
          ? new FloatLiteral(Number(num))
          : new IntegerLiteral(Number(num));
      }
