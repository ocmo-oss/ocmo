...
function formatPythonDict(data: Record<string, string>): string | null {
  ...
  const lines = entries.map(
    ([key, value]) =>
      `        ${formatPythonString(key)}: ${formatPythonString(value)},`,
  );
  return `{\n${lines.join("\n")}\n    }`;
}
...
export function buildResolveCliCommand(config: ResolveCommandConfig): string {
  ...
  if (config.isDraft) {
    ...
    parts.push(
      "resolve",
      shellQuote(
        cliAddress(config.path, config.mode ?? "config", config.versionRef),
      ),
    );
  }
  ...
  if (config.ignoreConfigsWithMissingTags)
    unsupported.push("ignore-configs-with-missing-tags");
...
export function buildResolveSdkCommand(config: ResolveCommandConfig): string {
  const lines = ["from ocmo import OcmoClient", "", "client = OcmoClient()"];
  ...
  if (config.isDraft) {
    ...
    lines.push("");
    lines.push(
      `result = client.ns(${formatPythonString(config.namespace)}).resolve_draft_config(`,
    );
    ...
    if (params) {
      lines.push(
        "# Draft dynamic params are sent as query params; use curl for the exact HTTP call.",
      );
    }
    return lines.join("\n");
  }
  ...
  lines.push(
    `result = client.ns(${formatPythonString(config.namespace)}).resolve(`,
  );
  lines.push(`    ${formatPythonString(config.path)},`);
  if (kwargs.length > 0) {
    lines.push(...kwargs);
