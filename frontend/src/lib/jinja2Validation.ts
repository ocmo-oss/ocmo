...
const BLOCK_OPEN_TAGS = new Set([
  'for', 'if', 'block', 'macro', 'filter', 'call', 'autoescape', 'with', 'raw',
])
...
export type TemplateValidationResult =
  | { valid: true }
  | { valid: false; issue: TemplateValidationIssue }

export function utf8ByteLength(value: string): number {
  return new TextEncoder().encode(value).length
}

export function validateJinja2Template(content: string): TemplateValidationResult {
  if (!content.trim()) {
    return {
      valid: false,
      issue: {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Invalid Jinja2 syntax'
    const issueMessage = message.startsWith('Invalid Jinja2') ? message : `Invalid Jinja2 syntax: ${message}`
    return {
      valid: false,
      issue: issueAtOffset(content, findLikelyErrorOffset(content, message), issueMessage),
    }
  }
}

function issueAtOffset(source: string, offset: number, message: string): TemplateValidationIssue {
...
function tagExtentAt(source: string, offset: number): { line: number; column: number } {
...
function findBlockStructureIssue(source: string): { offset: number; message: string } | null {
  ...
  while ((match = STATEMENT_TAG_RE.exec(source)) !== null) {
    ...
    if (expectedOpen) {
      ...
      if (!top || top.name !== expectedOpen) {
        if (top) {
          const expectedClose = BLOCK_OPEN_TO_CLOSE[top.name] ?? `end${top.name}`
          return {
            offset,
  ...
  if (unclosed) {
    const expectedClose = BLOCK_OPEN_TO_CLOSE[unclosed.name] ?? `end${unclosed.name}`
    return {
...
function findCharacterInJinjaRegion(source: string, needle: string): number | null {
...
export function applyTemplateValidationMarkers(
  ...
  const resolvedEndColumn = endColumn ?? Math.max(startColumn + 1, endLineContent.length + 1)
