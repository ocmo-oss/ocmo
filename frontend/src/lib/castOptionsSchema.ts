...
export function castOptionFieldLabel(key: string, prop: JsonSchemaProperty): string {
  if (prop.title?.trim()) return prop.title.trim()
  return key.replace(/_/g, ' ')
}

export function castOptionEnumValues(prop: JsonSchemaProperty): unknown[] | null {
...
export function castOptionPlaceholder(prop: JsonSchemaProperty): string | undefined {
  if (prop.default !== undefined && prop.default !== null && prop.default !== '') {
    return String(prop.default)
  }
  const example = prop.examples?.find(value => value !== undefined && value !== null && value !== '')
  if (example !== undefined) return String(example)
  return undefined
}

export function formatCastOptionDefault(prop: JsonSchemaProperty): string | undefined {
