...
function collapsedCaret(
  position: number,
): Pick<MarkdownEditResult, "selectionStart" | "selectionEnd"> {
...
function caretOnPrefixedLine(
  ...
  return (
    lineStart +
    offsetWithinBlock(lines, lineIndex) +
    prefix.length +
    lineText.length
  );
}
...
function prefixLines(
  ...
  if (!hadSelection) {
    ...
    return {
      value: newValue,
      ...collapsedCaret(
        caretOnPrefixedLine(lineStart, lines, lineIndex, prefix),
      ),
    };
  }
