...
export function createItemEditorModelPath(
  namespace: string,
  kind: string,
): string {
...
function collectDefs(
  ...
  if (
    schemaDefs &&
    typeof schemaDefs === "object" &&
    !Array.isArray(schemaDefs)
  ) {
...
export function buildConfigEditorSchema(
  ...
  const metadataSchema = filterMetadataSchemaForEditor(
    ocmoSchema,
    isJsonSchemaMode,
  );
  ...
  if (
    effectiveDataSchema?.required &&
    Array.isArray(effectiveDataSchema.required)
  ) {
