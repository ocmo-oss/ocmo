...
import { auditCategoryToApiFilter, formatAuditFieldValue, formatAuditOperation, type AuditCategory } from './auditEvent'
...
export const AUDIT_COLUMNS: AuditColumnDef[] = [
  ...
  { id: 'auth_email', label: 'Actor email', filterKey: 'auth_email', filterPlaceholder: 'email' },
  { id: 'auth_id', label: 'Actor ID', filterKey: 'auth_id', filterPlaceholder: 'subject' },
  {
    id: 'auth_type',
    label: 'Actor type',
    filterKey: 'auth_type',
  ...
  { id: 'token_number', label: 'Token #', filterKey: 'token_number', filterType: 'number' },
  { id: 'namespace', label: 'Namespace', filterKey: 'namespace', globalOnly: true },
  {
    id: 'event_kind',
    label: 'Event kind',
  ...
  { id: 'object_id', label: 'Object ID', filterKey: 'object_id', filterPlaceholder: 'path prefix' },
  { id: 'object_version', label: 'Object version', filterKey: 'object_version', filterType: 'number' },
  {
    id: 'subresource_type',
  ...
  { id: 'subresource', label: 'Subresource', filterKey: 'subresource', filterPlaceholder: 'latest' },
  {
    id: 'http_method',
    label: 'HTTP method',
  ...
  { id: 'api_endpoint', label: 'API endpoint', filterKey: 'api_endpoint', filterPlaceholder: '/api/v1/...' },
  { id: 'permission_ok', label: 'Outcome', filterKey: 'permission_ok', filterType: 'boolean' },
  {
    id: 'resolve_type',
    label: 'Resolve type',
  ...
  { id: 'from_cache', label: 'From cache', filterKey: 'from_cache', filterType: 'boolean' },
  { id: 'parent_event_id', label: 'Parent event', filterKey: 'parent_event_id', filterPlaceholder: 'uuid' },
  ...
  { id: 'id', label: 'Event ID', filterKey: 'event_id', filterPlaceholder: 'uuid' },
]
...
const COLUMN_MAP = Object.fromEntries(AUDIT_COLUMNS.map(col => [col.id, col])) as Record<AuditColumnId, AuditColumnDef>
...
export function parseAuditSearchParams(params: URLSearchParams): AuditUrlState {
  ...
  return {
    category: category === 'resolve' || category === 'modifications' ? category : 'all',
    columns,
    filters,
    offset,
  }
...
export function visibleAuditColumns(columns: AuditColumnId[], isGlobal: boolean): AuditColumnDef[] {
  return columns
...
export function renderAuditCell(columnId: AuditColumnId, event: AuditEvent): ReactNode {
  switch (columnId) {
    case 'operation':
...
export function auditCellTitle(columnId: AuditColumnId, event: AuditEvent): string | undefined {
  ...
  const value = columnId === 'id' ? event.id : event[columnId as keyof AuditEvent]
