...
import {
  ...
  fetchConfigMetadataSchema,
} from "../api/schema";
import { permissionsConfigPath } from "../lib/builtinPaths";
import {
  buildConfigEditorSchema,
  configEditorModelPath,
} from "../lib/configEditorSchema";
...
export function useConfigYamlSchema({
  ...
  const isPermissionsPolicyConfig =
    path === permissionsPolicyPath || path === PERMISSIONS_POLICY_CONFIG_PATH;
  ...
  const dataSchemaQuery = useQuery({
    ...
    enabled:
      hasSavedVersion && (!isJsonSchemaMode || isPermissionsPolicyConfig),
  });
  ...
  const composedSchema = useMemo(() => {
    ...
    const useExplicitDataSchema =
      isPermissionsPolicyConfig && Boolean(dataSchemaQuery.data);
