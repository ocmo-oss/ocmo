...
interface NotificationsState {
  ...
  closeTray: () => void
  toggleTray: () => void
  push: (severity: Severity, message: string, detail?: string, auditEventId?: string) => void
  signalSuccess: () => void
...
export function formatNotificationCopy(
  ...
  if (notification.auditEventId) lines.push(`Audit event ID: ${notification.auditEventId}`)
  return lines.join('\n')
}
...
export const useNotifications = create<NotificationsState>(set => ({
  ...
  signalSuccess: () => set(s => ({ successPulseGeneration: s.successPulseGeneration + 1 })),

  push: (severity, message, detail, auditEventId) => {
    set(state => {
      const existing = state.notifications.find(
        n => n.severity === severity && n.message === message && n.detail === detail,
      )
      if (existing) {
        const notification = {
          ...existing,
  ...
  clearModalNotification: modalId =>
    set(state => {
      ...
      const { [modalId]: _removed, ...modalNotifications } = state.modalNotifications
...
export function pushApiError(title: string, error: unknown, detailOverride?: string) {
  if (isApiUnavailableError(error)) {
    const detail = detailOverride ?? (error instanceof ApiError ? error.detail : undefined)
    pushNotification('error', 'API unavailable', detail)
    return
  }
  if (error instanceof ApiError) {
    pushNotification('error', title, detailOverride ?? error.detail, error.auditEventId)
    return
  }
  const detail = detailOverride ?? (error instanceof Error ? error.message : undefined)
