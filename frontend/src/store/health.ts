...
import type { BuiltinNamespacePaths, HealthResponse, ReservedTags } from '../api/types'
import {
...
export const useHealthStore = create<HealthState>(set => ({
  ...
  clearAvailabilityError: () => set(state => (
    state.healthError ? { healthError: null } : state
  )),
  applyVersionResponse: version => set(pickVersionBootstrap(version)),
  setVersionBootstrapFallback: () => set({
    version: 'unknown',
...
export function useConfigMetadataKey(): string {
  return useHealthStore(state => state.configMetadataKey) ?? DEFAULT_CONFIG_METADATA_KEY
}
