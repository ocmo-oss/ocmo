...
export function mapNavigationResponse(raw: RawNavigationResponse): NavigationResponse {
  return {
...
function mapVersionedNode(raw: RawBaseNode, type: 'config' | 'template'): ConfigNode
function mapVersionedNode(raw: RawBaseNode, type: 'secret'): SecretNode
function mapVersionedNode(raw: RawBaseNode, type: 'config' | 'template' | 'secret'): ConfigNode | SecretNode {
...
export function mapExtendedNode(raw: RawBaseNode): AnyExtendedNode {
  switch (raw.node_type) {
    ...
    case 'resolver': {
      const createdAt = raw.created_at ?? raw.version_data?.updated_at ?? new Date(0).toISOString()
...
export function mapVersionHistoryResponse(raw: RawVersionHistoryResponse): VersionHistoryResponse {
  return {
    path: raw.item.path,
...
function mapResolveTrace(trace?: Record<string, unknown>): Record<string, ResolveParticipant> | undefined {
  ...
  for (const [key, value] of Object.entries(trace)) {
    ...
    mapped[key] = {
      resource_type: (meta.resource_type as ResolveParticipant['resource_type']) ?? 'config',
      path: typeof meta.path === 'string' ? meta.path : path,
      version: typeof meta.version === 'number' ? meta.version : versionFromKey,
      resolve_role: (meta.resolve_role as ResolveParticipant['resolve_role']) ?? 'transitive',
