...
import { isTreeFolderAutoExpanded, isTreeNodeVisible } from '../../lib/treeFilter'
...
function FolderChainLabel({
  ...
  if (segments.length === 1) {
    return <span className="whitespace-nowrap font-mono">{segments[0].name}</span>
  }
...
export function TreeNodeComponent({
  ...
  const { namespace, '*': currentItemPath } = useParams<{ namespace: string; '*': string }>()
  ...
  const rowClass = 'group flex items-center gap-1 rounded-sm py-0.5 pr-2 hover:bg-slate-200 dark:hover:bg-gray-800'
  ...
  if (!isFolder) {
    ...
    return (
      <div className={rowClass} style={{ paddingLeft: `${indent + 8}px` }}>
        ...
        <TreeItemLabel item={item} namespace={namespace!} href={href} isFolder={false} />
  ...
  const nodeVisible =
    segments.some(segment => isTreeNodeVisible(segment.path, matchingPaths, searchActive))
    || children.some(child => isTreeNodeVisible(child.path, matchingPaths, searchActive))

  if (!nodeVisible) return null

  const autoExpanded = searchActive && (
    isTreeFolderAutoExpanded(terminalPath, matchingPaths)
    || segments.some(segment => isTreeFolderAutoExpanded(segment.path, matchingPaths))
  )
  const routeExpanded = !!currentItemPath
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <TreeItemLabel
            ...
            isFolder
          />
          {terminalLockInfo && <TreeLockIndicator lockInfo={terminalLockInfo} />}
        </div>
      </div>
      ...
      {expanded && (
        <div>
          {chainLoading && <SkeletonList count={2} itemClassName="h-5 w-full ml-6" />}
          {!chainLoading && visibleChildren.map(child => (
            <TreeNodeComponent
              key={child.path}
              item={child}
              depth={depth + 1}
          ...
          {!chainLoading && visibleChildren.length === 0 && (
            <p className="px-2 py-0.5 text-xs text-gray-400" style={{ paddingLeft: `${indent + 24}px` }}>
