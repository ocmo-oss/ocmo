...
export function TreeNav() {
  ...
  const { canRead: canReadLocks, isReady: lockPermissionsReady } = useLockPermissions(namespace)
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: ({ signal }) => treeApi.navigate(namespace!, null, { limit: 200 }, signal),
    enabled: !!namespace,
    staleTime: 30_000,
    gcTime: 5 * 60_000,
  })
  ...
  const { data: searchResults, isLoading: searchLoading } = useQuery({
    ...
    queryFn: ({ signal }) => treeApi.search(namespace!, null, { q: debouncedQuery, limit: 200 }, signal),
  ...
  const locks = lockPermissionsReady && canReadLocks ? (locksData?.locks ?? []) : []

  if (isLoading) return <SkeletonList count={6} itemClassName="h-6 w-full" />
  if (error) return <p className="p-2 text-xs text-red-500">Failed to load tree</p>
  ...
  const noSearchResults = searchActive && !searchLoading && matchingPaths.size === 0
  ...
  return (
    <nav aria-label="Configuration tree" className="min-w-max py-1">
      {searchActive && searchLoading ? (
        ...
        <p className="px-3 py-2 text-xs text-gray-400">No results for "{debouncedQuery}"</p>
      ) : (
        <>
          {canWrite && (
