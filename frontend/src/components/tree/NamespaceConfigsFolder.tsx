...
export function NamespaceConfigsFolder({
  ...
  const visibleItems = items.filter(item => isTreeNodeVisible(item.path, matchingPaths, searchActive))
  if (items.length === 0 || (searchActive && visibleItems.length === 0)) return null
    ...
    <div>
      ...
      >
        ...
        <div
          ...
          onClick={() => setExpanded(e => !e)}
        >
          <FolderCog className="h-4 w-4 shrink-0 text-amber-500" aria-hidden="true" />
      ...
      {expanded && (
        <div>
          {visibleItems.length === 0 && (
            <p className="px-2 py-0.5 text-xs text-gray-400" style={{ paddingLeft: '32px' }}>
              Empty
            </p>
          )}
