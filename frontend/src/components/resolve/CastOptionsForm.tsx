...
export function CastOptionsForm({ schema, values, onChange, className }: CastOptionsFormProps) {
  const properties = (schema.properties ?? {}) as Record<string, JsonSchemaProperty>
  const keys = Object.keys(properties)
  if (keys.length === 0) {
    return <p className="text-[10px] text-gray-400">No options for this format.</p>
  }
  ...
  return (
    <div className={cn('space-y-2', className)}>
      {schemaDescription && (
        <p className="text-[10px] leading-snug text-gray-400">{schemaDescription}</p>
      )}
      {keys.map(key => {
        const prop = properties[key]!
        const fieldState = getCastOptionFieldState(key, prop, properties, values)
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0">
                <span>{label}</span>
                {description && (
                  <span className="mt-0.5 block text-[9px] leading-snug text-gray-400">{description}</span>
                )}
                {fieldState.reason && (
        ...
        if (options) {
          return (
            <div key={key} className={disabledClass}>
              ...
              {description && <p className="mb-0.5 text-[9px] leading-snug text-gray-400">{description}</p>}
              {fieldState.reason && (
