...
export default function SecretView({
  ...
  const [placeholderContent, setPlaceholderContent] = useState(pickDummySecretContent)
  ...
  const [contentHydrated, setContentHydrated] = useState(() => isNew || revealFromNavigation)
  ...
  const { data: revealedItem, isFetching: revealing, error: revealError, refetch: refetchReveal } = useQuery({
    queryKey: ['item-reveal', namespace, item.path, item.version],
    queryFn: ({ signal }) => treeApi.get(namespace, item.path, { reveal: true, version: String(item.version) }, signal),
    enabled: revealed && !isNew && permissions.canRead,
    staleTime: 0,
    gcTime: 0,
  })

  const revealedContent = revealed && revealedItem?.type === 'secret' ? revealedItem.content : undefined
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      ...
      void qc.invalidateQueries({ queryKey: ['item-reveal', namespace, item.path] })
  ...
  const isSecretLoaded = isNew || revealedContent !== undefined || (revealed && contentHydrated)
  const showSave = !isDeleted && canEdit && isSecretLoaded
  const revealedEditorValue = contentHydrated ? content : (revealedContent ?? '')
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className={cn(
          ...
          {isDirty && activeTab === 'editor' && !isDeleted && isSecretLoaded && (
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          {activeTab === 'editor' && !isDeleted && (
            !permissions.canRead ? (
              ...
              <div className="relative flex min-h-0 flex-1 flex-col">
                {isSecretLoaded ? (
                  <MonacoEditor
                    key={creating ? 'revealed-create' : `revealed-${item.path}-${item.version}`}
                    height="100%"
                    language="yaml"
                    theme={monacoTheme}
                  ...
                  <>
                    <div className="min-h-0 flex-1 pointer-events-none select-none blur-[3px]">
                      <MonacoEditor
                        key={creating ? 'placeholder-create' : `placeholder-${item.path}-${item.version}`}
                        height="100%"
                        language="yaml"
                        theme={monacoTheme}
          ...
          {activeTab === 'audit' && (
            <ItemAuditTab namespace={namespace} path={item.path} type="secret" />
          )}
        </div>
      </div>
