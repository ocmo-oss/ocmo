...
export function VersionTagSelector({
  ...
  const versions = useMemo((): VersionEntry[] => {
    const merged = [...versionsFromApi];

    if (
      !searchQuery &&
      !merged.some((version) => version.version === currentVersion)
    ) {
      merged.unshift({
        version: currentVersion,
        tags: tags
          .filter((tag) => tag.version === currentVersion)
          .map((tag) => tag.name),
        updater: "",
        created_at: "",
        deleted_at: null,
  ...
  const showListLoading =
    isSearchPending || (isLoading && versionsFromApi.length === 0);
  ...
  useEffect(() => {
    setHighlighted((current) =>
      Math.min(current, Math.max(navItems.length - 1, 0)),
    );
  }, [navItems.length]);
                ...
                if (item.kind === "latest") {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={dropdownMetaClass}>
                        v{currentVersion}
                      </span>
                    </button>
                  );
                ...
                return (
                  ...
                  >
                    <span
                      className={cn(
                        "shrink-0 font-mono",
                        item.deletedAt && "text-gray-400 line-through",
                      )}
                    >
                      v{item.version}
                    </span>
                    {item.deletedAt && (
                      <Badge variant="error" className={dropdownTagClass}>
                        deleted
                      </Badge>
                    )}
