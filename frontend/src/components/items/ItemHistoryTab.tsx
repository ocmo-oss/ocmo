...
import { isReservedTagName, reservedTagsForItemType } from '../../store/versionBootstrap'
...
export function ItemHistoryTab({
  ...
  const {
    ...
    queryFn: ({ pageParam = 0, signal }) =>
      treeApi.versions(namespace, path, { limit: PAGE_SIZE, offset: pageParam }, signal),
    initialPageParam: 0,
  ...
  const { data: diffData, isLoading: diffLoading } = useQuery({
    ...
    queryFn: ({ signal }) => treeApi.diff(namespace, path, {
      from: diffPair!.from,
      to: diffPair!.to,
    }, signal),
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className={cn('flex min-h-0 overflow-hidden', diffPair ? 'hidden' : 'flex-1')}>
              ...
              {filteredTags.map(tag => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={e => { e.stopPropagation(); setVersionRef(tag.name) }}
                      className="shrink-0 hover:opacity-80"
                    >
                      <TagBadge name={tag.name} className="text-xs px-2 py-0.5" />
                    </button>
                    <button
                      type="button"
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="divide-y dark:divide-gray-700">
              {filteredVersions.map(version => {
                ...
                const isCurrent = versionRef === String(version.version)
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <button
                        type="button"
                        onClick={e => { e.stopPropagation(); setVersionRef(String(version.version)) }}
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {version.tags.map(tag => (
                            <button
                              key={tag}
                              type="button"
                              onClick={e => { e.stopPropagation(); setVersionRef(tag) }}
                              className="inline-flex"
                            >
                              <TagBadge name={tag} className="text-xs px-2 py-0.5" />
                            </button>
                          ))}
                          {version.deleted_at && <Badge variant="error">deleted</Badge>}
                        </div>
                        <p className="mt-1 text-xs text-gray-400">
                          {version.updater} · {formatUserDateTime(version.created_at)}
                        </p>
                      </button>
                      <div className="flex shrink-0 items-center gap-0.5">
                        {canTag && (
                          <Button
                            ...
                            onClick={e => {
                              e.stopPropagation()
                              setTaggingVersion(v => v === version.version ? null : version.version)
                            }}
                            title="Add tag"
                          >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => onDiffWithCurrent(String(version.version))}
                          title="Compare with current"
                        >
                          <GitCompare className="h-3.5 w-3.5" />
                        </Button>
                    ...
                    {taggingVersion === version.version && (
                      <InlineTagForm
                        ...
                        onSave={tag => tagMut.mutate({ tag, version: version.version })}
                      />
                    )}
                  </div>
                )
        ...
        <div className="flex min-h-0 flex-1 flex-col border-t dark:border-gray-700">
          <div className="flex shrink-0 items-center justify-between px-4 py-2">
            ...
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="sm" onClick={onSwapDiff} title="Swap versions">
                <ArrowLeftRight className="h-3.5 w-3.5" />
              </Button>
              <Button variant="ghost" size="sm" onClick={onCloseDiff}>Close</Button>
            </div>
          </div>
          {diffLoading && <Skeleton className="mx-4 mb-4 h-32 w-full shrink-0" />}
          {diffData && renderUnifiedDiff(diffData.from.content ?? '', diffData.to.content ?? '', path)}
        </div>
      )}
    </div>
  )
