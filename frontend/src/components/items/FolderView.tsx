    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-0.5 border-b px-4 dark:border-gray-700">
        {([
          ...
          ...(permissions.canResolve ? [{ id: 'resolve' as const, label: 'Resolve' }] : []),
      ...
      {activeTab === 'contents' && (
        <div className="flex-1 overflow-auto p-6">
          {!permissions.canRead ? (
            ...
            <>
              <h2 className="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-300">Contents</h2>
              ...
              <div className="grid gap-2">
                {data?.children.map(child => (
                  ...
                  >
                    ...
                    <span className="font-mono text-sm text-gray-800 dark:text-gray-200">{child.name}</span>
                  </Link>
                ))}
              </div>
            </>
      ...
      <DeleteDialog namespace={namespace} path={item.path} open={deleteOpen} onClose={() => setDeleteOpen(false)} />
      <LocationDialog
        mode="move"
        namespace={namespace}
        path={item.path}
