...
export default function ResolverView({
  ...
  const [activeTab, setActiveTab] = useState<Tab>(() =>
    createdTokenFromNavigation ? "tokens" : "config",
  );
  ...
  const [newToken, setNewToken] = useState<string | null>(
    () => createdTokenFromNavigation ?? null,
  );
  const [isInitialToken, setIsInitialToken] = useState(() =>
    Boolean(createdTokenFromNavigation),
  );
  ...
  const canSave =
    permissions.canWrite && (item.version === 0 || isDirty) && isHealthy();
  ...
  const rotateMut = useMutation({
    mutationFn: (tokenNumber: 1 | 2) =>
      treeApi.rotateToken(namespace, item.path, { token_number: tokenNumber }),
    onSuccess: (data) => {
      invalidateItemDetailQueries(qc, namespace, item.path);
  ...
  return (
    <div className="flex h-full flex-col">
        ...
        <div className="flex items-center gap-0.5 border-b px-4 dark:border-gray-700">
          {(
            [
              "config",
              "tokens",
              ...(permissions.canAudit ? ["audit" as const] : []),
            ] as Tab[]
          ).map((t) => (
            <button
              key={t}
          ...
          <div className="p-6 space-y-6">
            {(item.author || item.created_at) && (
              <p className="text-xs text-gray-500">
                ...
                {formatUserDateTime(item.created_at)}
              </p>
            )}
            <div className="rounded-lg border border-slate-300 p-4 dark:border-gray-700 space-y-4">
              ...
              <p className="text-xs text-gray-500">
                Tokens are only shown in full immediately after creation or
                rotation. Use two tokens for zero-downtime rotation.
              </p>
              {([1, 2] as const).map((n) => (
                <div key={n} className="space-y-1">
                  ...
                  <p className="pl-[3.75rem] text-xs text-gray-500">
                    Last used:{" "}
                    {formatTokenLastUsed(
                      n === 1 ? item.token1_last_used : item.token2_last_used,
                    )}
                  </p>
                </div>
              ))}
            ...
            {newToken && permissions.canWrite && (
              <div className="rounded-lg border-2 border-green-400 bg-green-50 p-4 dark:border-green-700 dark:bg-green-900/10">
                <p className="text-sm font-semibold text-green-800 dark:text-green-300">
                  {isInitialToken ? "Initial resolver token" : "New token"}{" "}
                  (shown once only — copy and store securely)
                </p>
      ...
      <Modal
        ...
        title={`Rotate token ${rotateConfirm}`}
        footer={
          <>
            <Button variant="ghost" onClick={() => setRotateConfirm(null)}>
              Cancel
            </Button>
            <Button
              variant="danger"
      ...
      >
        <p className="text-sm text-gray-600 dark:text-gray-400">
          A new token will be generated. The previous token {rotateConfirm} will
          be invalidated. The new token will be shown once — copy it before
          navigating away.
        </p>
      </Modal>
    </div>
