...
function parsePropagationPayload(
  subresource?: string | null,
): PropagationAuditPayload | null {
...
function renderPropagationTargets(targets: PropagationAuditTarget[]) {
  return targets.map((target, index) => (
    <span key={`${target.path}@${target.version}`}>
      {index > 0 && ", "}
      <TimelinePath path={target.path} />@
      <VersionBadge version={target.version} />
    </span>
  ));
}
...
export function AuditTimelineMessage({ entry }: { entry: AuditTimelineEntry }) {
  const subresources = parseSubresources(
  ...
  const tag =
    subresources.tag ??
    (entry.subresource_type === "tag" ? entry.subresource : null);
  ...
  switch (operation) {
    case "Create item":
      content =
        version != null ? (
          <>
            created {kind}{" "}
            {entry.object_id && <TimelinePath path={entry.object_id} />}{" "}
    ...
    case "Delete tag":
      content =
        tag && version != null ? (
          <>
            removed tag <TagBadge name={tag} /> from{" "}
            <VersionBadge version={version} />
          </>
        ) : tag ? (
          <>
    ...
    case "Copy item": {
      ...
      content =
        destination && copyTag ? (
          <>
            copied {kind} to <TimelinePath path={destination} /> (tag{" "}
            <TagBadge name={copyTag} />)
          </>
        ) : destination ? (
          <>
      ...
      if (trigger === "manual") {
        content =
          version != null ? (
            updated.length > 0 ? (
              <>
                manually propagated <VersionBadge version={version} /> to{" "}
                {renderPropagationTargets(updated)}
              </>
            ) : unchanged.length > 0 ? (
              <>
                manually propagated <VersionBadge version={version} />; all
                targets already matched ({renderPropagationPaths(unchanged)})
              </>
            ) : (
              <>
        ...
        content =
          version != null ? (
            updated.length > 0 ? (
              <>
                propagated by setting tag <TagBadge name={triggerTag} /> to{" "}
                <VersionBadge version={version} />, creating{" "}
                {renderPropagationTargets(updated)}
              </>
            ) : unchanged.length > 0 ? (
              <>
                propagated by setting tag <TagBadge name={triggerTag} /> to{" "}
                <VersionBadge version={version} />; all targets already matched
                ({renderPropagationPaths(unchanged)})
              </>
            ) : (
              <>
                propagated by setting tag <TagBadge name={triggerTag} /> to{" "}
        ...
        content = (
          <>
            propagated <VersionBadge version={version} /> of {kind}
          </>
        );
      } else {
    ...
    case "Promote stable tag": {
      ...
      content = (
        <>
          promoted tag <TagBadge name={promotedTag} /> on {kind}
        </>
      );
      break;
    }
    case "Rotate token": {
      ...
      content = token ? (
        <>rotated resolver token {token}</>
      ) : (
        <>rotated resolver token</>
      );
      break;
    }
    case "Create lock":
