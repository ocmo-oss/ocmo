...
export function LocationDialog({
  ...
  const moveMut = useMutation({
    mutationFn: () => treeApi.move(namespace, path, { target_path: normalizedTarget }),
    onSuccess: async () => {
  ...
  const tagOptions = ['latest', ...tags.map(t => t.name).filter(name => name !== 'latest')]
    ...
    >
      <div className="space-y-4">
        ...
        <PathSearchCombobox
          ...
          onSelect={folder => {
            setTargetPath(buildDestinationPathFromFolder(folder.path, itemName))
          }}
        />
        ...
        {isMove && MOVE_WARNING_TYPES.has(type) && (
          <div className="flex gap-2 rounded-md border border-yellow-300 bg-yellow-50 px-3 py-2 text-xs text-yellow-800 dark:border-yellow-800/60 dark:bg-yellow-900/20 dark:text-yellow-300">
            ...
            <ul className="list-disc space-y-1 pl-4">
              <li>
                Relative paths in <span className="font-mono">{configMetadataKey}</span> blocks (extend, render,
                validation schema, secrets, etc.) are resolved from this item&apos;s folder and may
                break after the move
                {type === 'folder' ? ', including configs inside this folder' : ''}.
                Review dependent configs before confirming.
              </li>
              <li>
                Resolve statistics are keyed by path and do not carry over after a move — the chart
                will be empty until new resolve activity is recorded at the destination path.
              </li>
            </ul>
          </div>
        )}
        ...
        {!isMove && showsCopyTagField(type, isMove) && (
          <div className="space-y-1">
            <label htmlFor="tag-to-copy" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              Version tag to copy
            </label>
            {type === 'folder' ? (
              <Input
        ...
        {normalizedTarget && (
          <p className="text-sm text-gray-600 dark:text-gray-400">
            ...
            <span className="font-mono font-medium text-gray-800 dark:text-gray-200">{itemName}</span>
            {' '}will be available as{' '}
            <span className="font-mono font-medium text-gray-800 dark:text-gray-200">{normalizedTarget}</span>
            .
          </p>
        )}
      </div>
