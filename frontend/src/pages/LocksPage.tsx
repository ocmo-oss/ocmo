...
function CreateLockModal({ namespace, open, onClose }: { namespace: string; open: boolean; onClose: () => void }) {
  ...
  const expiresAtError = expiresAt && !isOptionalFutureLocalDateTimeInput(expiresAt, minExpiresDate)
    ? 'Expiry must be at least 30 minutes from now'
    : undefined

  const mut = useMutation({
    mutationFn: () => locksApi.create(namespace, path.trim(), {
      reason,
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        />
        <Input label="Reason" value={reason} onChange={e => setReason(e.target.value)} placeholder="Deploying…" />
        <Input
      ...
      <div className="space-y-3 text-sm">
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2">
          {[
            ...
            ['Expires', lock.expires_at ? formatUserDateTime(lock.expires_at) : 'Never'],
          ].map(([k, v]) => (
...
export function LocksPage() {
  const { namespace, '*': splat } = useParams<{ namespace: string; '*': string }>()
  ...
  const {
    ...
    enabled: !!namespace && !!lockPath && !fromList && !!data && lockPermissions.isReady && lockPermissions.canRead,
    staleTime: 15_000,
    retry: false,
  })
  ...
  return (
    <div className="flex h-full flex-col">
        ...
        {data && (
          <table className="w-full table-fixed divide-y divide-slate-200 dark:divide-gray-800">
            ...
            <thead className="bg-surface dark:bg-gray-800/50 sticky top-0">
              <tr>
                {['Path', 'Reason', 'Locked by', 'Created', 'Expires', ''].map((h, i) => (
                  <th
                    key={h || `actions-${i}`}
                ...
                >
                  ...
                  <td className="px-2 py-2.5 text-right">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      loading={deleteMut.isPending && deleteMut.variables?.path === lock.path}
                      onClick={e => {
                        e.stopPropagation()
      ...
      <CreateLockModal namespace={namespace!} open={createOpen} onClose={() => setCreateOpen(false)} />
