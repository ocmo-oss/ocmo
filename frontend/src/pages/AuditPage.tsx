...
import { extractAuditEventIdInput, applyAuditEventIdFilter } from '../lib/auditEventIdSearch'
import {
  AUDIT_COLUMNS,
  auditCellTitle,
  auditFiltersToApi,
...
function patchFilter(filters: AuditFilters, key: AuditFilterKey, value: string): AuditFilters {
  const next = { ...filters }
  if (!value) delete next[key]
  else if (key === 'permission_ok' || key === 'from_cache') next[key] = value === 'true'
      ...
      <div className="space-y-4 text-sm">
        ...
        {event.event_kind === 'resolve_request' && (
          <div>
            ...
            {participantsLoading && <Skeleton lines={3} className="h-8 w-full" />}
            {!participantsLoading && participants.length === 0 && (
              <p className="text-xs text-gray-400">No participant rows recorded for this resolve.</p>
            )}
            {!participantsLoading && participants.length > 0 && (
              <div className="divide-y rounded border dark:divide-gray-700 dark:border-gray-700">
                {participants.map(participant => (
                  <div key={participant.id} className="flex flex-wrap items-center gap-2 px-3 py-2 text-xs">
                    ...
                    <Badge variant={participant.resolve_type === 'direct' ? 'info' : 'default'}>
                      {participant.resolve_type ?? '—'}
                    </Badge>
                    {participant.object_version != null && (
                      <span className="text-gray-400">v{participant.object_version}</span>
                    )}
                    {participant.from_cache && <Badge variant="success">cached</Badge>}
                  </div>
                ))}
              </div>
            )}
...
function ColumnPickerModal({
  ...
  return (
    <Modal
      ...
      footer={
        <>
          <Button variant="ghost" onClick={() => setDraft([...DEFAULT_AUDIT_COLUMNS])}>Reset</Button>
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          <Button
            variant="primary"
            disabled={draft.length === 0}
            onClick={() => {
              onApply(draft.length > 0 ? draft.slice(0, MAX_AUDIT_COLUMNS) : [...DEFAULT_AUDIT_COLUMNS])
              onClose()
            }}
          >
            Apply
      ...
      <div className="space-y-4">
        <div>
          <p className="mb-2 text-xs text-gray-500 dark:text-gray-400">
            Order below is left-to-right in the table. Use arrows to reorder.
            {' '}Up to {MAX_AUDIT_COLUMNS} columns can be shown at once
            {atColumnLimit ? ' (limit reached)' : ` (${draft.length}/${MAX_AUDIT_COLUMNS})`}.
          </p>
          {draft.length === 0 ? (
        ...
        {hiddenColumns.length > 0 && (
          <div>
            <p className="mb-2 text-xs text-gray-500 dark:text-gray-400">Add column</p>
...
function ColumnHeaderFilter({
  ...
  if (filterType === 'boolean' || filterType === 'select') {
    const options = filterType === 'boolean'
      ? [
          { value: '', label: 'any' },
  ...
  return (
    <input
      type={filterType === 'datetime' ? 'datetime-local' : filterType === 'number' ? 'number' : 'text'}
      value={value}
      onChange={e => onChange(filterKey, e.target.value)}
...
export function AuditTable({ namespace, isGlobal = false, eventId, scope }: AuditTableProps) {
  ...
  const urlState = useMemo(() => parseAuditSearchParams(searchParams), [searchParams])
  ...
  const effectiveFilters = useMemo(() => {
    if (filtersKey(draftFilters) === filtersKey(debouncedFilters)) return debouncedFilters
  ...
  useEffect(() => {
    ...
    setSearchParams(prev => {
      ...
      if (filtersKey(state.filters) === filtersKey(debouncedFilters)) return prev
      return buildAuditSearchParams({ ...state, filters: debouncedFilters, offset: 0 })
    }, { replace: true })
  }, [debouncedFilters, draftFilters, isScoped, setSearchParams])

  const scopedColumns = useMemo(() => ITEM_AUDIT_DEFAULT_COLUMNS, [])
  const visibleColumns = useMemo(
    () => visibleAuditColumns(isScoped ? scopedColumns : urlState.columns, isGlobal),
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['audit', namespace ?? 'global', isScoped ? 'item' : 'page', scope?.objectId, listFilters],
    queryFn: ({ signal }) =>
      isGlobal
        ? auditApi.listGlobal({ ...listFilters, limit: PAGE_SIZE }, signal)
        : auditApi.listNamespace(namespace!, { ...listFilters, limit: PAGE_SIZE }, signal),
    enabled: isGlobal || !!namespace,
    staleTime: 15_000,
    placeholderData: keepPreviousData,
  })

  const selectedFromList = activeEventId ? data?.items.find(item => item.id === activeEventId) : undefined
  ...
  const selectedEvent = activeEventId ? (selectedFromList ?? fetchedEvent ?? null) : null
  ...
  const { data: participantsData, isLoading: participantsLoading } = useQuery({
    ...
    queryFn: ({ signal }) =>
      isGlobal
        ? auditApi.listGlobal({ parent_event_id: selectedEvent!.id, limit: 200 }, signal)
        : auditApi.listNamespace(namespace!, { parent_event_id: selectedEvent!.id, limit: 200 }, signal),
  ...
  const hasClearableFilters =
    filtersKey(draftFilters) !== '{}' || (!isScoped && urlState.category !== 'all')
        ...
        <div className="flex flex-wrap items-center gap-2 border-b px-4 py-2 dark:border-gray-700">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <input
              ...
              onChange={e => setFilterValue('event_id', extractAuditEventIdInput(e.target.value))}
            ...
            <Button variant="ghost" size="sm" onClick={clearFilters} disabled={!hasClearableFilters}>
              Clear filters
            </Button>
          </div>
        </div>
        ...
        {data && (
          <table className="w-full table-fixed divide-y divide-slate-200 dark:divide-gray-800">
            ...
            <tbody className="divide-y divide-slate-200 bg-surface-elevated dark:divide-gray-800/50 dark:bg-gray-900">
                  ...
                  {visibleColumns.map(col => (
                    <td
                      ...
                      className={cn(
                        ...
                        col.id === 'operation' || col.id === 'api_endpoint' || col.id === 'user_agent'
              ...
              {data.items.length === 0 && (
                <tr>
                  <td colSpan={visibleColumns.length || 1} className="px-4 py-12 text-center text-sm text-gray-400">
...
export function NamespaceAuditPage() {
  const { namespace, eventId } = useParams<{ namespace: string; eventId?: string }>()
  return (
    <div className="flex h-full flex-col">
      <div className="border-b px-4 py-3 dark:border-gray-700">
        <h1 className="text-base font-semibold text-gray-900 dark:text-gray-100">
          Audit log — <span className="font-mono text-brand-600">{namespace}</span>
        </h1>
      </div>
...
export function GlobalAuditPage() {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b px-4 py-3 dark:border-gray-700">
        <h1 className="text-base font-semibold text-gray-900 dark:text-gray-100">Global audit log</h1>
        <p className="text-xs text-gray-400">Cross-namespace — global administrators only</p>
      </div>
      <div className="flex-1 overflow-hidden">
