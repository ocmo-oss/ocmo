...
const VyshyvankaNotFound = lazy(() =>
  import("../components/items/VyshyvankaNotFound").then((m) => ({
    default: m.VyshyvankaNotFound,
  })),
);
...
export function ItemDetailPage() {
  const { namespace, "*": path } = useParams<{
    namespace: string;
    "*": string;
  }>();
  ...
  useEffect(() => {
    ...
    const detail =
      itemError instanceof ApiError && itemError.status === 404
  ...
  if (
    !permissions.canRead ||
    (itemError && isPermissionDeniedError(itemError))
  ) {
    return (
      <ItemRestrictedView
        namespace={namespace!}
