...
import {
  registerYamlSchemaCompletion,
  installYamlCompletionTriggers,
} from "../lib/yamlCompletion";
...
function RuleEditorModal({
  ...
  const userClaims =
    whoami?.auth_type === "user" ? whoami.user_details.claims : undefined;
  ...
  const completionDisposableRef = useRef<
    import("monaco-editor").IDisposable | null
  >(null);
  const suggestTriggerDisposableRef = useRef<
    import("monaco-editor").IDisposable | null
  >(null);
  const emptyHintDisposableRef = useRef<
    (import("monaco-editor").IDisposable & { refresh: () => void }) | null
  >(null);

  const attachSchemaCompletion = useCallback(
  ...
  const handleEditorMount = (
    ...
    emptyHintDisposableRef.current = installYamlEditorEmptyHint(
      editorInstance,
      monaco,
      {
        hasSuggestions: () => true,
...
export function GlobalPermissionsPage() {
  ...
  const moveMut = useMutation({
    mutationFn: ({ id, pos }: { id: string; pos: number }) =>
      permissionsApi.move(id, { position: pos }),
    onSuccess: () =>
      void qc.invalidateQueries({ queryKey: ["global-permissions"] }),
  });
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8">
      ...
      <div className="mb-6 rounded-lg border border-slate-300 bg-surface px-4 py-3 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-800/40 dark:text-gray-400">
        <p>
          Global permissions control which OIDC users may access which
          namespaces. Each rule matches a namespace name or glob pattern (for
          example <span className="font-mono">prod</span> or{" "}
          <span className="font-mono">team-*</span>) and grants one or more
          gates to matching users. Rules are evaluated top to bottom; the first
          match wins. Access is denied by default.
        </p>
        <ul className="mt-2 list-disc space-y-1 pl-5 text-xs">
          <li>
            ...
            — see the namespace in listings, open it, and browse items. Actions
            on individual items still depend on that namespace&apos;s{" "}
          ...
          <li>
            ...
            — everything read allows, plus edit namespace settings and the
            namespace <span className="font-mono">_permissions</span> policy.
          </li>
          <li>
            <strong className="font-medium text-gray-700 dark:text-gray-300">
              delete
            </strong>{" "}
            — delete the namespace and all of its contents.
          </li>
          <li>
            <strong className="font-medium text-gray-700 dark:text-gray-300">
              audit
            </strong>{" "}
            — view the audit log for the namespace.
          </li>
        </ul>
        <p className="mt-2 text-xs">
          Global administrators always have{" "}
          <span className="font-mono">read</span> and{" "}
          <span className="font-mono">audit</span> on every namespace without a
          matching rule. <span className="font-mono">write</span> and{" "}
          <span className="font-mono">delete</span> still require an explicit
          rule, even for global administrators. Only global administrators can
          edit these rules.
        </p>
      </div>
        ...
        {!isLoading && (data?.rules.length ?? 0) === 0 && (
          <div className="rounded-lg border border-dashed p-12 text-center dark:border-gray-700">
            <p className="text-sm text-gray-400">
              No rules defined — all access is denied by default.
            </p>
          </div>
        )}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-1 mt-1">
                  {rule.rule.read && (
                    <span className="text-xs bg-blue-50 text-blue-700 px-1.5 rounded dark:bg-blue-900/20 dark:text-blue-300">
                      read
                    </span>
                  )}
                  {rule.rule.write && (
                    <span className="text-xs bg-green-50 text-green-700 px-1.5 rounded dark:bg-green-900/20 dark:text-green-300">
                      write
                    </span>
                  )}
                  {rule.rule.delete && (
                    <span className="text-xs bg-red-50 text-red-700 px-1.5 rounded dark:bg-red-900/20 dark:text-red-300">
                      delete
                    </span>
                  )}
                  {rule.rule.audit && (
                    <span className="text-xs bg-slate-200 text-gray-600 px-1.5 rounded dark:bg-gray-800">
                      audit
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-1">
                <Button
                  ...
                  disabled={idx === 0}
                  onClick={() =>
                    moveMut.mutate({ id: rule.id, pos: rule.position - 1 })
                  }
                >
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  disabled={idx === (data?.rules.length ?? 0) - 1}
                  onClick={() =>
                    moveMut.mutate({ id: rule.id, pos: rule.position + 1 })
                  }
                >
