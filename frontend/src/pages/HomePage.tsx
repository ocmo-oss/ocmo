    ...
    >
      <div className="space-y-3">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          ...
          <strong className="font-mono">{ns?.name}</strong> and all its configs,
          templates, secrets, and resolvers. This action cannot be undone.
        </p>
        <Input
...
export function HomePage() {
  ...
  const { canCreate: canCreateNamespace, isLoading: createPermLoading } =
    useCanCreateNamespace();
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: ({ signal }) =>
      namespacesApi.list(
        { name_filter: filter || undefined, limit: 50 },
        signal,
      ),
    staleTime: 30_000,
  });
          ...
          {isGlobalAdmin && (
            <>
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => navigate("/permissions/global")}
              >
      ...
      >
        {!isLoading && data?.items.length === 0 && (
          <div className="rounded-lg border border-dashed border-slate-400 p-12 text-center dark:border-gray-700">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {filter
                ? `No namespaces matching "${filter}"`
                : "No namespaces yet"}
            </p>
            <p className="mt-1 text-xs text-gray-400">
              Access is controlled by Global Permissions. Contact your platform
              administrator if you expect to see namespaces here.
            </p>
          </div>
        )}
            ...
            <table className="min-w-full divide-y divide-slate-300 dark:divide-gray-700">
              <thead className="bg-surface dark:bg-gray-800/50">
                <tr>
                  ...
                  <th className="px-4 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-gray-500 hidden sm:table-cell">
                    Description
                  </th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-gray-500 hidden md:table-cell">
                    Created
                  </th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-gray-500 hidden md:table-cell">
                    Updated
